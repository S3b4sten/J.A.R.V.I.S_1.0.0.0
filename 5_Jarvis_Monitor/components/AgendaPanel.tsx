    ...
    <div className="w-full h-full bg-black/40 border border-cyan-500/20 p-2 backdrop-blur-md relative overflow-hidden animate-in slide-in-from-bottom-4 duration-700 flex flex-col">
      ...
      <div className="mb-1.5 p-1 bg-cyan-500/5 border-l-2 border-cyan-400 flex justify-between items-center shrink-0">
        <div className="min-w-0">
          <div className="text-[5.5px] font-mono text-cyan-500/60 uppercase tracking-tighter">Active</div>
          <div className="text-[8.5px] font-orbitron text-cyan-50 uppercase truncate leading-tight tracking-wider">Fusion_Maint</div>
        </div>
        <div className="text-right ml-2 shrink-0">
          <div className="text-[5.5px] font-mono text-cyan-400/40 uppercase">ETA</div>
          <div className="text-[8px] font-orbitron text-cyan-200">00:42:15</div>
        </div>
      </div>
      
      <div className="flex-1 space-y-1 overflow-y-auto no-scrollbar pr-1">
        {MOCK_AGENDA.map((item, idx) => (
          <div key={idx} className="flex items-center gap-2 border-b border-white/5 pb-0.5">
            <div className="w-7 text-[7px] font-mono text-cyan-500/40 tabular-nums shrink-0">{item.time}</div>
