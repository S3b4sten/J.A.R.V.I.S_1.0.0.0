...
const IntelReadout: React.FC<IntelReadoutProps> = ({ value, isVisible }) => {
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-top-4 duration-700 z-40">
      <div className="relative group">
        ...
        <div className={`bg-black/80 backdrop-blur-2xl border ${isFault ? 'border-red-500/40' : 'border-cyan-500/20'} p-6 flex flex-col items-center shadow-[0_0_80px_rgba(0,212,255,0.15)] relative overflow-hidden transition-colors duration-500`}>
          ...
          <div className={`flex items-center gap-3 mb-4 border-b ${isFault ? 'border-red-500/40' : 'border-cyan-500/20'} pb-2 w-full justify-center relative`}>
            ...
            <span className={`font-orbitron text-[9px] tracking-[0.5em] ${isFault ? 'text-red-400' : 'text-cyan-400'} uppercase opacity-80`}>
              {isFault ? 'System_Fault_Detected' : 'Data_Extraction_Module'}
            </span>
          </div>
          
          <div className="flex flex-col items-center relative z-10">
            <span className={`text-[9px] font-mono ${isFault ? 'text-red-500/40' : 'text-cyan-500/40'} uppercase tracking-[0.3em] mb-2`}>
              {isFault ? 'Diagnostics:' : 'Analysis_Result:'}
            </span>
            <span className={`text-3xl md:text-5xl font-mono font-bold ${isFault ? 'text-red-100' : 'text-cyan-50'} tracking-tight text-center leading-tight glow-text-sharp transition-colors duration-500`}>
              {value}
            </span>
          </div>
          ...
          <div className="mt-6 flex justify-between items-center w-full">
            ...
            <span className={`text-[8px] font-mono tracking-widest uppercase ${isFault ? 'text-red-500/40' : 'text-cyan-500/40'}`}>
